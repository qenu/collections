    ...
    async def format_command_help(
        ...
        if await ctx.embed_requested():
            ...
            if command_help:
                ...
                name = splitted[0]
                value = "\n\n".join(splitted[1:])
                emb["fields"].append(EmbedField("敘述:", name[:250], False))
            else:
                value = ""
            emb["fields"].append(EmbedField("使用方式:", signature, False))

            if aliases := get_aliases(command, ctx.invoked_with):
                emb["fields"].append(EmbedField("別稱", ", ".join(aliases), False))
            ...
            if value:
                emb["fields"].append(EmbedField("完整敘述:", value[:1024], False))
