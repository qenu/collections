...
from .utils import (close_menu, emoji_converter, first_page, get_aliases,
                    get_cooldowns, get_perms, home_page, last_page, next_page,
                    prev_page, react_page, shorten_line)
...
class BaguetteHelp(commands.RedHelpFormatter):
    ...
    async def format_category_help(
        ...
        **kwargs,
        ...
        if await ctx.embed_requested():
            ...
            await ctx.send(_("你必須要使用embeds來開啟選單功能"))
    ...
    async def format_command_help(
        ...
        signature = _(
            "`語法: {ctx.clean_prefix}{command.qualified_name} {command.signature}`"
        ...
        if await ctx.embed_requested():
            ...
            if command_help:
                ...
                if alias := get_aliases(command, ctx.invoked_with):
                    emb["fields"].append(EmbedField("別稱", ",".join(alias), False))

                if final_perms := get_perms(command):
                    emb["fields"].append(EmbedField("權限", final_perms, False))

                if cooldowns := get_cooldowns(command):
                    emb["fields"].append(EmbedField("冷卻時間", "\n".join(cooldowns), False))
    ...
    async def format_bot_help(
        ...
        if await ctx.embed_requested():
            ...
            for i in pagify(
                "\n".join(
                    f"{str(cat.reaction) if cat.reaction else ''} `{ctx.clean_prefix}幫助 {cat.name:<10}:`**{cat.desc}**\n"
                    for cat in filtered_categories
                    if cat.cogs
                ),
                page_length=1018,
            ):
                emb["fields"].append(EmbedField("分類:", i, False))
    ...
    async def make_embeds(
        ...
        author_info = {
            "name": _("{ctx.me.display_name} 幫助選單").format(ctx=ctx),
        ...
        for i, group in enumerate(field_groups, 1):
            ...
            if page_count > 1:
                description = _("第{page_num}頁 | {page_count}\n{content_description}").format(
